...
- Cat food/cans only count verified users toward pricing: `getGlobalBalances` (pool counter build/rebuild in `model/cat-can.ts`) matches `realname_flag >= 1`, and the 清理未认证猫资产 button on `/oi33/admin` (`purgeUnverifiedCatAssets`, flag >= 3) destroys every unverified user's cat food (logged per-user as `cat_account`/`deduct`, reason 未认证用户资产清理), returns their cans to the reserve pool (`circulatingCans` decreases, reserve food unchanged), removes their small cat from the arena (`oi33_cat_map_player` delete + `remove` broadcast) and wipes their big-cat contributions — both the current one (the bound cat's `currentWeight` is decremented, NOT moved to history; the binding itself is kept) and all archived feed history (`oi33_school_feed_history` rows deleted, each cat's `historyWeight` decremented accordingly, so re-verified users have nothing to restore); affected cats get a `bigcat` broadcast. Downgrading a verified user does NOT auto-purge — rerun the button to bring the incremental counters back in sync.
...
- **Regular user** editing self → `oi33Model.submitRequest()` creates a `pending` doc in `oi33_request`; `oi33_user` is unchanged until approval. Existing pending for the same `uid` + `kind` is marked `cancelled` (both the request doc and its activity-log entry), so the old log line shows "已取消" instead of staying "待审批". Unverified users (`realname_flag < 1`) may only submit `realname` requests — every other kind is rejected server-side and the edit page replaces those forms with a notice.
...
| `/oi33/admin/cat-assets/purge` | AdminCatAssetPurgeHandler (POST) | OI33 flag >= 3 |
| `/oi33/admin/mail-blacklist` | AdminMailBlacklistHandler (GET 列表 / POST 添加) | OI33 flag >= 2 |
| `/oi33/admin/mail-blacklist/:domain/delete` | AdminMailBlacklistDeleteHandler (POST) | OI33 flag >= 2 |
...
### Mail blacklist
- `/oi33/admin/mail-blacklist` manages Hydro's built-in `mail::<domain>` blacklist entries (collection `blacklist`), which the core checks at registration (`UserRegisterHandler`), OAuth first-login callback and email change (`HomeSecurityHandler.postChangeMail`) — the core ships the check but no UI. Added entries use `BlackListModel.add(id, 0)` (≈permanent) and take effect immediately; existing accounts are unaffected. There is no built-in whitelist mechanism.
