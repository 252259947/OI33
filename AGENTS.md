...
- **Trade contracts** (`/oi33/contracts`): an achievement definition marked `saleable` (checkbox in 成就管理) can be resold by a user whose award came from an auction (`source: 'auction'`). The seller creates a contract naming a buyer UID and a cat-food price; one pending contract per seller+achievement. The named buyer accepts, declines, or the seller cancels. On accept the buyer pays the full price (sufficiency gate) while the seller receives the price minus a 5% intermediary fee (`contractFeeAmount`, ceiled, stored as `fee` on the contract); the fee is burned and the AMM pool's `userFoodTotal` counter is decremented to match. The seller's auction award is deleted and re-granted to the buyer with `source: 'contract'` — contract-bought copies cannot be resold. The creation form carries a prominent fee notice, and the outgoing/resolved tables show the fee and the seller's net proceeds. All actions log `type: 'contract'` (accept logs include `fee`).
