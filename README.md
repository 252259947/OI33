...
| 成就交易合同 | 标记为「可售卖」的成就被拍卖获得后，可挂给指定用户按约定猫粮数量转售；对方接受即成交（成就转让、猫粮转账） | `/oi33/contracts` |
...
- 每天 00:00、08:00、16:00（Asia/Shanghai）根据全站用户猫粮、有效罐头、真实储备和虚拟可用罐头重新计算卖出目标价。目标价不会高于“储备 ÷ 流通罐头”的全额兑付上限，每个 8 小时时段最多变动 3%。
...
- 成就管理页可为成就勾选「可售卖」。只有**通过拍卖获得**（`source: 'auction'`）且被标记可售卖的成就才能转售；合同买来的成就（`source: 'contract'`）不能再次转售。
