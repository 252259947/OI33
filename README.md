...
- 点击任意格会弹出格子详情，列出位于该格的全部小猫及余额，并提供加入、移动或传送按钮；点击自己所在格时还可使用 0～255 的 xterm 8-bit 调色板设置当前格颜色。改色不消耗猫粮或罐头，但会触发共享冷却；颜色在小猫离开后永久保留。
- 所有小猫适用完全相同的消耗和共享冷却规则，管理员小猫没有移动特权；位置允许重叠，同一格只绘制猫粮最多的小猫（猫粮相同时 UID 较小者优先）。管理员绘图权限仅在独立后台使用，可按（行,列）指定单像素或包含边界的矩形范围；坐标从左上角 `(0,0)` 到右下角 `(479,639)`。
- 未涂色地图使用纯白背景。
- 地图使用统一的公共颜色底图和连续缩放视野，前端提供“格线”“小猫”“名字”三个独立显示开关；定位、坐标跳转和全屏位于单独的导航工具栏。关闭显示开关不会改变位置、倍率或点击操作。低倍率下格线会自动按间隔抽样，避免线条挤成色块；发生名字遮挡时优先显示猫粮更多的用户。桌面端可拖动、滚轮缩放，并在全屏后平滑使用方向键浏览；移动端支持单指拖动和双指缩放。
