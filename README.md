...
- 向上下左右相邻空格移动销毁 33g 猫粮；传送到任意其他格消耗 3 个猫罐头，罐头从用户库存和流通量中扣除并回到虚拟储备池。
...
- 未涂色地图使用纯白背景。
- 地图使用统一的公共颜色底图和连续缩放视野，前端提供“格线”“小猫”“名字”三个独立显示开关；定位、坐标跳转和全屏位于单独的导航工具栏。关闭显示开关不会改变位置、倍率或点击操作。低倍率下格线会自动按间隔抽样，避免线条挤成色块；发生名字遮挡时优先显示罐头更多的用户。桌面端可拖动、滚轮缩放，并在全屏后平滑使用方向键浏览；移动端支持单指拖动和双指缩放。
...
- 行政管理员可在绘图后台按 UID 将指定用户的小猫强制迁移到随机空位；该操作不消耗用户资源，也不会改变其共享冷却或免冷却染色额度。
