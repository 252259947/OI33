...
| 猫粮与罐头账户 | 签到奖励、管理员发放（自动跳过未认证用户）、罐头交易、统一账本与 7 天余额曲线 | `/oi33/cat-account/:uid` |
| 猫罐头市场 | 储备池动态定价、每日买卖、永久持有与简洁交易界面 | `/oi33/cat-can` |
| 猫猫广场 | 640×480 实时网格世界，小猫移动、传送与 8-bit 公共像素涂色；底部展示罐头实时价格（含 24h 涨跌）与进行中成就拍卖 | `/oi33/arena` |
| 大猫世界 | 每所学校一只大猫：绑校、烧猫粮投喂养大（1024 g 起上图，体重翻倍体型 +8），榜首可摆放位置；当前/历史投喂双榜 | `/oi33/arena` |
...
| 成就系统 | 8/16/24/32 像素 PNG；支持通过题数、连续/累计登录、猫粮/罐头余额五类阈值自动发放；AC 统计域可全局配置并按题号跨域去重；个人页提供 4×4 成就展示柜（用户自选最多 16 个徽章，可自由排序）和紧凑成就列表面板（最多 50 条，可跳全部成就页）；实时达成和手动授予时自动发布个人喵喵 | `/oi33/achievements` |
| 成就拍卖 | 管理员上架成就拍卖（时长 1–720 小时 + 起拍价），已认证用户用猫罐头出价、每次至少加 1 个；出价托管扣款、被超越自动退款、成交后赢家获得成就且罐头回流罐头池 | `/oi33/auction` |
...
| `oi33_school` | OIerDB 学校库（启动时按计数差异自动导入） |
| `oi33_school_cat` | 每只被投喂过的学校大猫：当前重量、历史重量、可选的固定位置 |
...
| `oi33_auction` | 成就拍卖：起拍价、起止时间、状态、当前最高价/领先者、赢家与成交价 |
...
| `/oi33/arena` | 公开 | 640×480 猫猫广场（未认证用户不显示） |
| `/oi33/arena/join` | `PRIV_USER_PROFILE` + 已认证 | 首次免费选择位置加入广场（POST） |
| `/oi33/arena/move` | `PRIV_USER_PROFILE` + 已认证 | 加入后移动或传送小猫（POST） |
| `/oi33/arena/color` | `PRIV_USER_PROFILE` + 已认证 | 设置自己小猫所在格颜色（POST，与移动共享冷却；移动或传送后首次染色可豁免） |
| `/oi33/arena/big/schools` | `PRIV_USER_PROFILE` + 已认证 | 大猫学校列表 / 搜索 |
| `/oi33/arena/big/bind` | `PRIV_USER_PROFILE` + 已认证 | 绑定 / 改绑学校大猫（POST，首次免费，之后每自然月一次） |
| `/oi33/arena/big/feed` | `PRIV_USER_PROFILE` + 已认证 | 烧猫粮投喂大猫（POST，2 小时冷却） |
| `/oi33/arena/big/cat/:schoolId` | 公开 | 大猫详情与当前/历史投喂榜 |
| `/oi33/arena/big/cat/:schoolId/position` | `PRIV_USER_PROFILE` + 已认证（仅当前投喂榜首） | 摆放大猫位置（POST，2 小时冷却） |
...
| `/oi33/auction/:id/bid` | `PRIV_USER_PROFILE` + 已认证 | 出价（POST，每次至少比当前最高价多 1 个罐头） |
...
- 管理员可给其他用户单人发放猫粮；行政管理员可粘贴 `[{"uid":123,"amount":500,"reason":"奖励"}]` 格式的 JSON，先生成 30 分钟有效预览，再确认一次性发放。
...
- 到期后惰性结算（有人访问拍卖列表/详情或出价时触发）：赢家直接获得成就（照常发布成就喵喵公告），其支付的罐头回流罐头池（不返还猫粮）；无人出价则流拍。管理员可取消进行中的拍卖并退还当前最高出价。
...
- 地图固定为 640×480 个格子。用户初始不会自动加入广场；已认证用户第一次可自行选择任意位置免费加入，这次不消耗资源也不触发冷却。多个用户可以位于同一格。未认证用户不会出现在地图中，也不能加入或操作；身份降为未认证时会立即移除其位置，再次认证后需要重新加入。
- 向上下左右相邻格移动销毁 3g 猫粮；传送到任意其他格消耗 3 个猫罐头，罐头从用户库存和流通量中扣除并回到虚拟储备池。
...
- 所有小猫适用完全相同的消耗和共享冷却规则，管理员小猫没有移动特权；位置允许重叠，同一格只绘制猫罐头最多的小猫（猫罐头相同时 UID 较小者优先）。管理员绘图权限仅在独立后台使用，可按（行,列）指定单像素或包含边界的矩形范围；坐标从左上角 `(0,0)` 到右下角 `(479,639)`。
- 未涂色地图使用纯白背景。
- 地图使用统一的公共颜色底图和连续缩放视野，前端提供“格线”“小猫”“名字”三个独立显示开关；定位、坐标跳转和全屏位于单独的导航工具栏。关闭显示开关不会改变位置、倍率或点击操作。低倍率下格线会自动按间隔抽样，避免线条挤成色块；发生名字遮挡时优先显示猫罐头更多的用户。桌面端可拖动、滚轮缩放，并在全屏后平滑使用方向键浏览；移动端支持单指拖动和双指缩放。
...
猫猫广场的同一张画布上还有一层「大猫」：每只大猫代表 OIerDB 的一所学校，公开标签为 `<拼音缩写>#<OIerDB编号>`（如 `CSSYLZX#0`），点击跳转到 `oier.baoshuo.dev` 的对应学校页面。学校数据来自 OIerDB（`scripts/school.txt`），用 `python scripts/build-school-cat-data.py`（需 `pypinyin`）重新生成后在启动时自动导入。

### 绑定与投喂

- 参与要求与小猫地图一致（`realname_flag ≥ 1`）。投喂后被降级的用户保留绑定和贡献，只是恢复认证前不能再投喂/改绑。
- 每个用户绑定一只学校大猫；**首次绑定免费**，之后每个自然月（Asia/Shanghai）可改绑一次。改绑时，你在旧猫上的累计贡献从「当前重量」移入「历史重量」并归档一条投喂历史；重新绑回投喂过的猫时，历史贡献会恢复为当前贡献。
- 投喂**销毁猫粮**：你的猫粮余额减少，大猫当前重量和你的个人贡献同步增加，并写入猫粮统一账本。投喂有固定 2 小时冷却。
- 大猫体重达到 **1024 g** 才会出现在地图上（8×8 格）；体重每翻一倍（2048、4096……）边长 +8。不足 1024 g 的猫保留数据但不显示。

### 位置与排行榜

- 大猫没有移动模拟。未摆放的猫由前端按种子伪随机、互不重叠地自动布局；每只猫的**当前投喂榜第一名**可每 2 小时摆放一次它的位置（固定左上角格子）。
- 每只猫提供「当前投喂榜」（来自现有绑定）和「历史投喂榜」（来自归档记录）两个排行榜，在猫的详情页查看。
- 体重和位置变化通过 WebSocket 实时广播，画布上三层绘制顺序为：涂色格子 → 大猫 → 小猫。
...
- **系统成就公告**：授予成就时直接生成一条已通过的个人喵喵，不扣罐头、不占每日免费次数，也不触发两小时冷却。
