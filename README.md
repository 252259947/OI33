...
| 猫粮与罐头账户 | 签到奖励、管理员发放、罐头交易、统一账本与 7 天余额曲线 | `/oi33/cat-account/:uid` |
| 猫罐头市场 | 储备池动态定价、每日买卖、永久持有与简洁交易界面 | `/oi33/cat-can` |
| 猫猫广场 | 640×480 实时网格世界，小猫移动、传送与 8-bit 公共像素涂色 | `/oi33/arena` |
...
| `oi33_cat_can_batch` | 猫罐头购买批次与剩余库存（不设过期时间） |
...
| `oi33_cat_map_player` | 已认证用户的小猫坐标、移动冷却和并发移动锁 |
...
| `/oi33/arena/join` | `PRIV_USER_PROFILE` + 已认证 | 首次免费选择空位加入广场（POST） |
| `/oi33/arena/move` | `PRIV_USER_PROFILE` + 已认证 | 加入后移动或传送小猫（POST） |
| `/oi33/arena/color` | `PRIV_USER_PROFILE` + 已认证 | 设置自己小猫所在格颜色（POST，与移动共享冷却） |
| `/oi33/cat-arena/admin` | OI33 身份 = 3 | 按（行,列）进行矩形绘图 |
| `/oi33/cat-arena/admin/relocate` | OI33 身份 = 3 | 按 UID 将小猫强制迁移到随机空位（POST） |
...
| `/oi33/cat-food/grant` | OI33 身份 ≥ 2 | 给其他用户单人发放猫粮 |
| `/oi33/cat-food/grant/bulk` | OI33 身份 = 3 | 粘贴 JSON、预览批量发放名单 |
| `/oi33/cat-food/grant/bulk/confirm` | OI33 身份 = 3 | 确认一次性批量发放（POST） |
...
- 猫罐头不设保质期，不会自动过期或销毁；卖出时按购买先后顺序扣减库存。
...
- 地图固定为 640×480 个格子。用户初始不会自动加入广场；已认证用户第一次可自行选择任意空位免费加入，这次不消耗资源也不触发冷却。未认证用户不会出现在地图中，也不能加入或操作；身份降为未认证时会立即释放原位置，再次认证后需要重新加入。
- 向上下左右相邻空格移动销毁 33g 猫粮；传送到任意其他格消耗 1 个猫罐头，罐头从用户库存和流通量中扣除并回到虚拟储备池。
- 移动、传送和修改当前格颜色共用冷却：`ceil(max(50, 120 × 0.95^sqrt(罐头数)))` 分钟。最低为 50 分钟；传送按扣除罐头后的余额计算。
- 点击自己的小猫可使用 0～255 的 xterm 8-bit 调色板设置当前格颜色。改色不消耗猫粮或罐头，但会触发共享冷却；颜色在小猫离开后永久保留。
- 所有小猫适用完全相同的消耗、共享冷却和唯一占位规则，管理员小猫没有移动特权。管理员绘图权限仅在独立后台使用，可按（行,列）指定单像素或包含边界的矩形范围；坐标从左上角 `(0,0)` 到右下角 `(479,639)`。
- 未涂色地图使用纯白背景。
- “网格地图”使用黑色格线并显示用户名、罐头数量和随格子同步缩放的小猫；发生名字遮挡时优先显示罐头更多的用户，只有该模式响应点击移动/改色。“像素地图”仅供浏览，只显示公共颜色图。两种模式都支持以鼠标位置为中心的滚轮连续缩放与拖动，并可输入行、列坐标直接居中跳转。
...
- 行政管理员可在绘图后台按 UID 将指定用户的小猫强制迁移到随机空位；该操作不消耗用户资源，也不会改变其共享冷却。
