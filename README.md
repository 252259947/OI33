...
新建训练仅对教练和管理员开放，并仍须具备 Hydro 原生 `PERM_CREATE_TRAINING` 权限。普通账号不显示列表顶部和空列表中的新建入口；服务端同时拦截直接 GET/POST 新建请求及伪造参数。已有训练的浏览、参加、做题，以及原有编辑/删除权限保持不变。

作业和比赛的新建/编辑页面可直接向题目选择框粘贴多个题号；训练的新建/编辑页面提供「按章节批量加题」，先建立章节，再选择目标章节并粘贴。支持英文/中文逗号、空格、换行和分号，每次最多 200 个，仅匹配当前域题库中有权查看的已有题目（不会从洛谷自动拉题）。全部匹配成功后按原顺序追加并按实际题目 ID 去重；任一题号不存在、无权限或查询失败时，本次不添加任何题目。查询期间暂停表单保存；训练计划在查询期间被修改时取消本次添加，防止覆盖编辑内容。添加后仍需保存作业、比赛或训练才会生效。

作业/比赛成绩表（含 HTML、CSV 导出）以及训练的参训学生、进度对比提示优先显示当前域已审批的真实姓名。没有核验姓名时沿用原来的昵称或匿名 UID；不会合并同名用户，也不改分数、排名、用户链接、班型权限或封榜规则。姓名只进入这些原本有权查看的成绩页面，不写入全站 User 缓存；训练作者、个人资料、讨论等仍用原有昵称规则。新实名档案为权威，待审/拒绝/跨域档案不得回退旧实名，只有不存在新档案且属于本域的用户才使用旧认证姓名。HTML 姓名自动转义，CSV 姓名按文本处理以避免公式执行。Ghost 团队交换格式保持原生行为。
...
- 猫粮余额显示在用户详情页的小猫组件下方；签到成功后直接返回首页展示今日运势和打卡天数，不弹出签到或猫粮奖励提示。
...
认证身份与实名信息相互独立：认证通过恢复公开用户名和头像；全站通用用户资料中的 `realname_name` 仍属于敏感信息，中括号实名及实名字段只对管理员和行政管理员显示。教学成绩页面是明确的局部例外：按上述成绩表规则展示已核验姓名，不放宽该成绩表的访问权限，也不附带学校、学号或审批信息。
