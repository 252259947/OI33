...
| 大猫世界 | 每所学校一只大猫：绑校、烧猫粮投喂，按每格绘图时的用户绑定归属领地；独立领地色、体重/格数榜与当前/历史投喂双榜 | `/oi33/arena` |
...
| `oi33_school_cat` | 已出现的学校大猫：当前/历史重量、不重复的领地颜色、占领格数 |
...
| `/oi33/arena/move` | `PRIV_USER_PROFILE` + 已认证 | 相邻移动 3g；同一当前大猫领地内任意传送 3g；其他传送 3 罐头（POST） |
...
- 向上下左右相邻格移动销毁 3g 猫粮。非相邻移动时，如果起点和终点都属于用户**当前绑定的同一只大猫**，则领地传送只销毁 3g 猫粮；其他传送消耗 3 个猫罐头，罐头从用户库存和流通量中扣除并回到虚拟储备池。
...
- `oi33_school_cat` 除当前/历史重量外，会按格子归属增量维护 `territoryCount`。每只猫提供「当前投喂榜」和「历史投喂榜」；广场侧栏同时显示体重和占领格数。
